...
Usage:
  Single-mode headless (2 users, 1/s spawn rate, 60s):
    locust -f tests/performance/test_track_load.py --headless -u 2 -r 1 -t 60s --host http://localhost:8050
  ...
  Web UI mode:
    locust -f tests/performance/test_track_load.py --host http://localhost:8050
    # Then open http://localhost:8089

  Reproducible runs (seeded random date selection):
    LOCUST_SEED=42 locust -f tests/performance/test_track_load.py --headless -u 2 -r 1 -t 60s --host http://localhost:8050
